...
// ── Inventory Table Tab ────────────────────────────────────────────────────
function InventoryTab({ inventory, onStockUpdate }) {
  ...
  const handleStockSubmit = async () => {
    await onStockUpdate(stockModal.productId, {
      ...
      quantity:        parseInt(stockForm.qty, 10),
      referenceNo:     stockForm.ref  || null,
    ...
    <div>
          ...
          <tbody>
            {filtered.map((item, i) => (
              <tr key={item.productId} style={{
                ...
                <td style={{ padding: "12px 16px", fontFamily: "monospace", color: "#6366f1", fontWeight: 700, fontSize: 12 }}>
                  {item.sku}
                </td>
                ...
                <td style={{ padding: "12px 16px" }}>
                  <span style={{
                    ...
                    color: item.currentStock === 0          ? "#ef4444"
                         : item.currentStock <= item.reorderLevel ? "#f59e0b"
                         : "#1e293b"
      ...
      {stockModal && (
        <Modal title={`Update Stock — ${stockModal.productName}`} onClose={() => setStockModal(null)}>
          ...
          />
          <Input label="Reference No (optional)"
            value={stockForm.ref}
...
function AddProductTab({ categories, onAdd }) {
  ...
  const handleSubmit = async () => {
    if (!form.sku || !form.name || !form.sellingPrice) {
      setStatus("❌ SKU, Name, and Selling Price are required.");
      return;
    }
    setLoading(true);
    try {
      ...
      setStatus("❌ Error adding product. Check console.");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div style={{ maxWidth: 600 }}>
      ...
      {status && (
        <div style={{
          padding: "10px 16px", borderRadius: 8, marginBottom: 16, fontWeight: 600,
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0 16px" }}>
        ...
        <Input label="GST %" type="number" min="0" max="100"
        ...
        <Input label="Initial Stock" type="number" min="0"
          value={form.initialStock}  onChange={e => set("initialStock",  e.target.value)} />
      </div>
...
export default function App() {
  ...
  const load = async () => {
    ...
    try {
      ...
      // Demo data so the UI is usable without a running backend
      setInventory([
        { productId:1, sku:"ELEC-001", productName:"USB-C Cable 1m",        categoryName:"Electronics", unit:"pcs",  sellingPrice:120, gstPercent:18, currentStock:120, reorderLevel:50, stockStatus:"IN_STOCK" },
        { productId:2, sku:"ELEC-002", productName:"Wireless Mouse",        categoryName:"Electronics", unit:"pcs",  sellingPrice:599, gstPercent:18, currentStock:35,  reorderLevel:20, stockStatus:"IN_STOCK" },
        { productId:3, sku:"STAT-001", productName:"A4 Paper Ream",         categoryName:"Stationery",  unit:"ream", sellingPrice:150, gstPercent:5,  currentStock:80,  reorderLevel:30, stockStatus:"IN_STOCK" },
        { productId:6, sku:"PACK-002", productName:"Bubble Wrap Roll 50m",  categoryName:"Packaging",   unit:"roll", sellingPrice:700, gstPercent:18, currentStock:8,   reorderLevel:10, stockStatus:"LOW_STOCK" },
        { productId:8, sku:"FG-001",   productName:"Customised Pen Drive 32GB", categoryName:"Finished Goods", unit:"pcs", sellingPrice:450, gstPercent:18, currentStock:0, reorderLevel:15, stockStatus:"OUT_OF_STOCK" },
      ]);
      setCategories([
      ...
      <div style={{ maxWidth: 1200, margin: "0 auto", padding: "28px 32px" }}>
        ...
        {error && (
          <div style={{
            ...
            background: "#fef3c7", color: "#92400e", fontWeight: 600, fontSize: 13
          }}>
            ⚠️ {error}
          </div>
        )}
        ...
        <div style={{ background: "#fff", borderRadius: 14, boxShadow: "0 1px 4px #0001", overflow: "hidden" }}>
          ...
          <div style={{ padding: 28 }}>
            {loading ? (
              ...
              <InventoryTab inventory={inventory} onStockUpdate={handleStockUpdate} />
            ) : tab === "add" ? (
